...
| # | Date | Modification |
|---|------|--------------|
| 10 | 2026-02-09 | **Cardinality enforcement: 3 optimized strategies**: Rewrote `cardinality.py` with 4 selectable methods (`auto`/`sequential`/`gradient`/`miqp`/`two_stage`). (1) **Gradient**: first-order Taylor ΔH_i ≈ \|∂H/∂w_i\| × w_i replaces O(\|S_sub\|) per-position entropy evals with single gradient call. (2) **MIQP**: single MOSEK mixed-integer QP with binary z_i for exact semi-continuous constraint — replaces entire eliminate-reoptimize loop. (3) **Two-stage** (DVT §4.7): analytical y* = c/√λ_k in factor space (targets H=ln(AU)), then MIQP tracking QP projection to asset space. Automatic fallback chain: two_stage→miqp→gradient→sequential. Added `has_mi_solver()` to `sca_solver.py`, `cardinality_method` to `PortfolioConfig`. 5 files modified, 7 new tests, pyright clean, 233 tests pass. |
